...
class TemporalFusionTransformer(object):
  ...
  def _build_base_graph(self):
    ...
    def get_lstm(return_state):
      ...
      if self.use_cudnn:
        lstm = tf.keras.layers.Bidirectional(tf.keras.layers.CuDNNLSTM(
            self.hidden_layer_size,
      ...
      return lstm
    #Andrej
    # print('STATIC CONTEXT VECTORS')
    # print(static_context_state_c)
    # print(static_context_state_h)
    # static_context_state_c = tf.expand_dims(static_context_state_c,axis=2)
    # static_context_state_h = tf.expand_dims(static_context_state_h,axis=2)
    # static_context_state_c = tf.tile(static_context_state_c, [1,1,64])
    # static_context_state_h = tf.tile(static_context_state_h, [1,1,64])
