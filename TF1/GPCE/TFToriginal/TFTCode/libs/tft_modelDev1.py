...
class TFTDataCache(object):
  ...
  def get(cls, key):
    ...
    if cls._data_cache[key] is None:
        return None
    else:
...
class SymbolicLayer(tf.keras.Model):
    def __init__(self):
    ...
    def call(self, start,  Lookupinputs, minibatch_size, time_steps, training=None):


#        utils.print_out_shape(start)
        tempall_inputs = tf.reshape(start,[-1])




        numinbatch = tempall_inputs.shape[0]
        if numinbatch is None:
            numinbatch = minibatch_size
            initialvalue = np.left_shift(time_steps + 1, 16)
            tempall_inputs = np.full(numinbatch, initialvalue, dtype=np.int32)
        else:  # tempall_inputs =
            tempall_inputs = tf.keras.backend.get_session().run(tempall_inputs)




        iseqarray = np.right_shift(tempall_inputs, 16)
        ilocarray = np.bitwise_and(tempall_inputs, 0b1111111111111111)
        print('iseq ', iseqarray.shape, ilocarray.shape, iseqarray[0])
        X_train_withSeq = list()
        for iloc in range(0, numinbatch):
            X_train_withSeq.append(
                Lookupinputs[ilocarray[iloc], iseqarray[iloc] - time_steps: iseqarray[iloc]])
        numpyall_inputs = np.array(X_train_withSeq)
        #        numpyall_inputs = np.zeros((numinbatch, time_steps,combined_input_size), dtype = np.float32)
        all_inputs = tf.convert_to_tensor(numpyall_inputs)
        return all_inputs
...
class TemporalFusionTransformer(object):
  ...
  def __init__(self, raw_params, TFTdfTotal = None,  use_cudnn=False):
    ...
    self.TFTLSTMFinalMLP = params['TFTLSTMFinalMLP']
    ...
    if self.TFTSymbolicWindows:
    # Set up Symbolic maps allowing location order to differ (due to sorting in TFT)
        ...
        self.Lookupinputs = np.zeros((self.TFTNloc, self.TFTnum_entries, self.input_size))
        for identifier, df in TFTdfTotal.groupby(id_col):
  ...
  def get_tft_embeddings(self, all_inputs):
    ...
    utils.print_out_shape(regular_inputs)
    utils.print_out_shape(obs_inputs)
  ...
  def _get_single_col_by_type(self, input_type):
    ...
    return utils.get_single_col_by_input_type(input_type,
                                              self.column_definition, self.TFTMultivariate)
  ...
  def cache_batched_data(self, data, cache_key, num_samples=-1):
    ...
    if num_samples > 0:
      TFTDataCache.update(
          self._batch_sampled_data(data, cache_key, max_samples=num_samples), cache_key)
    else:
  ...
  def _batch_sampled_data(self, data, cache_key, max_samples):
    ...
    for identifier, df in data.groupby(id_col):
      ...
      self.TFTSeq = max(self.TFTSeq, self.TFTnum_entries-self.time_steps+1)
      if self.TFTnum_entries >= self.time_steps:
        valid_sampling_locations += [
            (identifier, self.time_steps + i)
            for i in range(self.TFTnum_entries - self.time_steps + 1)
        ]
      split_data_map[identifier] = df
    print(cache_key + ' max samples ' + str(max_samples) + ' actual ' + str(len(valid_sampling_locations)))
    ...
    if self.TFTSymbolicWindows:
        ...
        outputs = np.zeros((actual_samples, self.time_steps, self.output_size))
        time = np.empty((actual_samples, self.time_steps, 1), dtype=object)
        identifiers = np.empty((actual_samples, self.time_steps, 1), dtype=object)
...
#         Sequence runs from start_idx - self.time_steps to start_idx i.e. start_idx is label of FINAL time step in position start_idx - 1
          if self.TFTMultivariate:
        ...
        inputs = np.zeros((actual_samples, self.time_steps, self.input_size))
        outputs = np.zeros((actual_samples, self.time_steps, self.output_size))
        time = np.empty((actual_samples, self.time_steps, 1), dtype=object)
        identifiers = np.empty((actual_samples, self.time_steps, 1), dtype=object)
  ...
  def _build_base_graph(self):
    ...
    # Inputs.
    if self.TFTSymbolicWindows:
        start = tf.keras.layers.Input(shape=(1,1),dtype=tf.int32)
        all_inputs = SymbolicLayer()(start,self.Lookupinputs, self.minibatch_size, self.time_steps)
    else:
    ...
    # utils.print_out_shape(all_inputs)
    print('all inputs', all_inputs.shape)


    unknown_inputs, known_combined_layer, obs_inputs, static_inputs \
        = self.get_tft_embeddings(all_inputs)
    utils.print_out_shape(unknown_inputs)
    utils.print_out_shape(known_combined_layer)
    utils.print_out_shape(obs_inputs)
    utils.print_out_shape(static_inputs)
    ...
    utils.print_out_shape(static_encoder)
    utils.print_out_shape(static_weights)
    ...
    if (self.output_size == 1) and (len(self.quantiles) == 1):
        self.TFTFinalGatingOption = 0
    if self.TFTFinalGatingOption > 0:
        self.TFTLSTMFinalMLP = 0
        self.FinalLoopSize = self.output_size * len(self.quantiles)
    ...
    if self.FinalLoopSize > 1:
        transformer_layer = tf.keras.backend.stack(StackLayers, axis=-1)
    print('self.FinalLoopSize  ', self.FinalLoopSize)
    print('Transformer_layer ', transformer_layer.shape)
  ...
  def build_model(self):
    ...
    with tf.variable_scope(self.name):
      ...
      if self.TFTLSTMFinalMLP > 0:
          outputs = apply_mlp(transformer_layer[Ellipsis, self.num_encoder_steps:, :],
              hidden_size = self.TFTLSTMFinalMLP,
              output_size = self.output_size * len(self.quantiles),
              output_activation = None,
          ...
          if self.FinalLoopSize == 1:
              ...
              outputstack =[]
              localloopsize = self.output_size * len(self.quantiles)
              for localloop in range(0,localloopsize):
                  localoutput = tf.keras.layers.Dense(1)(transformer_layer[Ellipsis, self.num_encoder_steps:, :, localloop])
                  outputstack.append(localoutput)
              outputs = tf.keras.backend.stack(outputstack, axis=-2)
      ...
      def custom_lossGCF8(y_actual, y_pred):
          ...
          tensordiff = tf.math.reduce_sum(tf.math.square(y_actual - y_pred), axis=-1)


          return tensordiff


      def custom_lossGCF9(y_actual, y_pred):
          tensordiff = tf.math.reduce_sum(tf.math.abs(y_actual - y_pred), axis=-1)
          return tensordiff
      def custom_lossGCF10(y_actual, y_pred):
          tensordiff = tf.math.reduce_sum(tf.math.abs(y_actual - y_pred))
          return tensordiff
      ...
      if self.useMSE:
          TFTLossFunction = 'MSE'
          if self.TFTOption2 == 0:
              TFTLossFunction = 'MSE'
          elif self.TFTOption2 == 1:
              TFTLossFunction = custom_lossGCF1A
          elif self.TFTOption2 == 8:
              TFTLossFunction = custom_lossGCF8
          elif self.TFTOption2 == 9:
              TFTLossFunction = custom_lossGCF9
          elif self.TFTOption2 == 10:
              TFTLossFunction = custom_lossGCF10
          elif self.TFTOption2 == 11:
              TFTLossFunction = 'MAE'
      else:
          TFTLossFunction = QuantileLossCalculator(valid_quantiles).quantile_loss


      if self.TFTOption1 == 1:
          model.compile(loss=TFTLossFunction, optimizer=adam)
      else:
          model.compile(loss=TFTLossFunction, optimizer=adam, sample_weight_mode='temporal')
  ...
  def fit(self, train_df=None, valid_df=None):
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ModelCheckpoint(
            filepath=self.get_keras_saved_path(self._temp_folder)+'-{epoch:02d}-{val_loss:.2f}',
            monitor='val_loss',
    ...
    if self.NumberQuantiles == 3:
        Predictedvalues = np.concatenate([labels, labels, labels], axis=-1)
    elif self.NumberQuantiles == 1:
        Predictedvalues = labels
    ...
    if valid_data is None:
        ...
        if self.NumberQuantiles == 3:
            PredictedValvalues = np.concatenate([val_labels, val_labels, val_labels], axis=-1)
        elif self.NumberQuantiles == 1:
            PredictedValvalues = val_labels
        if self.TFTOption1 == 1:
            self.model.fit(
                x=data,
                y=Predictedvalues,
                ...
                epochs=self.num_epochs,
                batch_size=self.minibatch_size,
                validation_data=(val_data, PredictedValvalues),
                callbacks=all_callbacks,
                shuffle=True,
                use_multiprocessing=True,
                workers=self.n_multiprocessing_workers)
            ...
            self.model.fit(
                ...
                epochs=self.num_epochs,
                batch_size=self.minibatch_size,
                validation_data=(val_data, PredictedValvalues, val_flags),
                callbacks=all_callbacks,
                shuffle=True,
                use_multiprocessing=True,
                workers=self.n_multiprocessing_workers)
  ...
  def evaluate(self, data=None, eval_metric='loss'):
    ...
    if self.NumberQuantiles == 3:
        Predictedoutputs = np.concatenate([outputs, outputs, outputs], axis=-1)
    elif self.NumberQuantiles == 1:
        Predictedoutputs = outputs
    if self.TFTOption1 == 1:
  ...
  #        'Current version only supports 1D targets!')
    ...
    def format_outputs(prediction):
      ...
      print('format outputs ', prediction.shape)
    ...
    process_map = {
        ...
        for i, qname in enumerate(self.Quantilenames)
    }
...
#    shutil.copytree(self._temp_folder,self._temp_folder1) # fails if tmp1 exists
