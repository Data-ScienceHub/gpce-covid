...
class PredictionProcessor:
    ...
    def convert_prediction_to_dict(self, predictions, index, target_time_step:int=None):
        ...
        if target_time_step is not None:
            assert 0 < target_time_step <= self.max_prediction_length, f"Expects target time step within 1 and {self.max_prediction_length}, found {target_time_step}."
    ...
    def align_result_with_dataset(self, df, predictions, index, target_time_step:int=None):
        ...
        merged_data = result_df.merge(df[self.targets + id_columns], on=id_columns, how='inner').reset_index(drop=True)
