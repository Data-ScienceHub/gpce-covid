...
# Generic.
def get_single_col_by_input_type(input_type, column_definition, TFTMultivariate):
  ...
  if TFTMultivariate and (input_type == 0):
    ...
    if len(l) != 1:
      raise ValueError('Invalid number of columns for Type {}'.format(input_type))
    return l[0]
...
def print_out_shape(var) -> None:
  if var is None:
    return
  current_stack = inspect.stack()
  current_file = current_stack[1][1]
  current_frame = inspect.currentframe()
  lineno = current_frame.f_back.f_lineno
  var_name = None
  for k, v in current_frame.f_back.f_locals.items():
    if v == var:
      var_name = k
      print(f"{current_file}:{lineno} {var_name} shape={var.shape}")
      break
