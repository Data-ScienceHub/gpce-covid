...
class TemporalFusionTransformer(object):
  ...
  def _batch_data(self, data):
    ...
    # #Andrej addition 1 - printing the data map to debug int_weights
    # print(data_map)
    # Combine all data
    for k in data_map:
  ...
  def _build_base_graph(self):
    ...
    def static_combine_and_mask(embedding):
      ...
      print('Post Softmax MLP outputs')
    ...
    '''
    Andrej Adding in some experiments with Attention
        1. Simple skip lstm and send tensor to multi head attention
    '''
    skip_lstm_concat = historical_inputs#concat([historical_inputs, future_inputs],axis=1)
    _, tsA, embdA, numInA = skip_lstm_concat.get_shape().as_list()
    skip_lstm_concat = tf.transpose(skip_lstm_concat,perm=[0,1,3,2])
    skip_lstm_concat = tf.reshape(skip_lstm_concat, [-1, tsA*numInA, embdA])
    new_attn_layer = InterpretableMultiHeadAttention(
        self.num_heads, self.hidden_layer_size, dropout=self.dropout_rate)
    mask_new = get_decoder_mask(historical_inputs)
    xn_save, attn_dist = new_attn_layer(skip_lstm_concat,skip_lstm_concat,skip_lstm_concat,
                                        mask=mask_new)
    print('New Attention to Save')
    print(xn_save.shape)
    '''
    End Andrej additions
    '''
    ...
    if (self.output_size == 1) and (self.quantiles == 1):
        self.TFTFinalGatingOption = 0
    if self.TFTFinalGatingOption > 0:
        self.TFTLSTMFinalMLP = 0
        self.FinalLoopSize = self.output_size
        if (self.quantiles == 1):
            self.TFTFinalGatingOption = 1
        if (self.TFTFinalGatingOption == 2):
            self.FinalLoopSize = self.FinalLoopSize * self.quantiles
